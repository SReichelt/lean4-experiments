...
namespace HasFunctorialArrows
  ...
  def arrow_congrFun  {α β : U} {F G : α ⟶ β} (a : α) : (F ⇝ G) ⟶ (F a ⇝ G a) := HasFunOp.swapFun h.arrowCongr (HasRefl.refl a)
  def arrow_congrFun' {α β : U} {F G : α ⟶ β} (a : α) : (F ⇝ G) → (F a ⇝ G a) := HasInternalFunctors.funCoe (arrow_congrFun U a)
...
namespace HasFunctorialEquivalences
  ...
  def equiv_congrFun  {α β : U} {F G : α ⟶ β} (a : α) : F ≃ G ⟶ F a ≃ G a := HasFunOp.swapFun h.equivCongr (HasRefl.refl a)
  def equiv_congrFun' {α β : U} {F G : α ⟶ β} (a : α) : F ≃ G → F a ≃ G a := HasInternalFunctors.funCoe (equiv_congrFun U a)
...
section Functors
  ...
  namespace compFun
    ...
    variable {U V W : Universe} [HasInstanceArrows V] [HasInstanceArrows W]
             [HasExternalFunctors U V] [HasExternalFunctors V W] [HasExternalFunctors U W]
             [HasCompFun U V W] [HasArrowCongrArg V W]
